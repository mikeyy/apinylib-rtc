...
class TinychatRTCClient(object):
    ...
    async def on_closed(self, code):
        ...
        if code == 4:
            ...
            self.console_write(COLOR['white'], f'Connection was closed, code: {code}')
    ...
    async def on_joined(self, client_info):
        ...
        client.user_level = 0
        self.console_write(COLOR['bright_green'], f'Client joined the room: {client.nick}, {client.id}')
    ...
    async def on_room_info(self, room_info):
        ...
        if config.DEBUG_MODE:
            ...
            self.console_write(COLOR['white'], '{:<50}'.format('## Room Information ##'))
            for k in room_info:
    ...
    async def on_userlist(self, user_info):
        ...
        if user_info['handle'] != self.client_id:
            ...
            if _user.is_owner:
                _user.user_level = 1
                self.console_write(COLOR['red'], f'Joins room owner: {_user.nick}:{_user.id}:{_user.account}')
            elif _user.is_mod:
                _user.user_level = 3
                self.console_write(COLOR['bright_red'], f'Joins moderator: {_user.nick}:{_user.id}:{_user.account}')
            elif _user.account:
                self.console_write(COLOR['bright_yellow'], f'Joins: {_user.nick}:{_user.id}:{_user.account}')
            else:
                self.console_write(COLOR['cyan'], f'Joins: {_user.nick}:{_user.id}')
    ...
    async def on_join(self, join_info):
        ...
        if _user.account:
            ...
            self.console_write(COLOR['cyan'],
                               f'{_user.nick}:{_user.id} joined the room')
    ...
    async def on_nick(self, uid, nick):
        ...
        old_nick = _user.nick
        _user.nick = nick
        self.console_write(COLOR['bright_cyan'], f'{old_nick}:{uid} Changed nick to: {nick}')
    ...
    async def on_quit(self, uid):
        ...
        if _user is not None:
            self.console_write(COLOR['cyan'], f'{_user.nick}:{uid} Left the room.')
    ...
    async def on_unban(self, unban_info):
        ...
        if unbanned is not None:
            self.console_write(COLOR['green'], f'{unbanned.nick} was unbanned.')
    ...
    async def message_handler(self, msg):
        ...
        # meh
        _msg = msg.replace('\n', ' ')
        self.console_write(COLOR['bright_green'], f'{self.active_user.nick}: {_msg}')
    ...
    async def private_message_handler(self, private_msg):
        ...
        """
        self.console_write(COLOR['green'], f'Private message from {self.active_user.nick}: {private_msg}')
    ...
    async def on_unpublish(self, uid):
        ...
        if _user is not None:
            ...
            self.console_write(COLOR['yellow'], f'{_user.nick}:{uid} stopped broadcasting.')
    ...
    async def on_pending_moderation(self, pending):
        ...
        if _user is not None:
            ...
            log.error(f"failed to find user info for green room pending user ID: {pending['handle']}")
    ...
    async def on_yut_play(self, yt_data):
        ...
        if yt_data['item']['offset'] == 0:
            ...
            self.console_write(COLOR['bright_magenta'],
    ...
    async def send_join_msg(self):
        ...
        if rtc_version is None:
            ...
            log.info(f'failed to parse rtc version, using fallback: {config.FALLBACK_RTC_VERSION}')
